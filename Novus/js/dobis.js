	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
		{
			...
			if (response) {
				...
				var tbody = document.getElementById('cd');
				if (tbody) tbody.innerHTML='';
		...
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
		{
			...
			if (response) {
				...
				for (var i = 0; i < response.length; i++) {
                    ...
                    view_td.innerHTML = "<a href='/view/"+response[i].username+"/' onclick='getUser("+currentstudentId+");return false;' $('#view').modal('show');>Edit</a>"					
                    row.appendChild(view_td);

					var pay_td = document.createElement("td");
					pay_td.innerHTML = "<a href='/edit_student/"+response[i].username+"/' onclick='getUser("+currentstudentId+");return false;' class='btn btn-default'><i class='edit icon'></i>update</a>"
					row.appendChild(pay_td);
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
	{
		...
		if (response) {
			...
			for (var i = 0; i < response.length; i++) {
				...
				category_td.innerHTML = response[i].category;
