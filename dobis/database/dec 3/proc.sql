...
BEGIN                                                             
	INSERT INTO users(`username`,`fname`, `lname`, `email`,`photo`, `password`, `CreateByusername`, `modifybyusername`)
	VALUES(username,fname, lname, email, photo, PASSWORD(p_password), CreateByusername, modifybyusername);
END $$
...
BEGIN	
	SELECT u.email,u.fname, u.photo, u.lname,l.login_time
	...
	IF (p_username AND p_password) IS NOT NULL THEN
		SELECT p_username as username, @on as email, @fn as firstname, @ln as lastname, @ph as photo, @lt as login_time FROM DUAL;
	END IF;
END $$ 
...
CREATE PROCEDURE viewB_location(IN p_title INT)
BEGIN
...
BEGIN
	INSERT INTO book_detail(`book_isbn`, `title`, `edition`,photo, `publisher`, `category`)
	VALUES(book_isbn, title, edition,photo, publisher, category);
END $$
...
BEGIN
	SELECT cd_title, cd_edition, cd_publisher, cd_category 
	FROM cd B
	LEFT JOIN cd_detail C
	ON C.cd_detail_isbn = B.cd_isbn
	WHERE cd_title = p_title;
END $$
