...
BEGIN
	INSERT INTO users(`username`,`fname`, `lname`, `email`,`password`, `CreateByusername`,`creation_date`, `modifybyusername`)
	VALUES(username,fname, lname, email, PASSWORD(p_password), CreateByusername, NOW(), modifybyusername);
END $$
...
DROP PROCEDURE IF EXISTS Asigningperm_rol $$
CREATE PROCEDURE Asigningperm_rol (IN p_username VARCHAR(50),
	IN p_password VARCHAR(255))
BEGIN
	GRANT CREATE, DELETE, UPDATE ON libdb.* TO username;
END $$
-- ---------------------------------------------------------------------------
				-- get permision
-- ---------------------------------------------------------------------------
...
BEGIN
	...
	LEFT JOIN permision_roles pr
	ON pr.permision_permission_id = p.permission_id
	LEFT JOIN roles r
...
BEGIN
	...
	SELECT u.username, u.email,u.photo, l.login_time
	FROM users u LEFT JOIN logtime l ON l.users_username=u.username
	WHERE u.username=p_username AND l.login_time=
		(select max(login_time) from logtime where username=p_username)
		into @fn,@on,@ph,@lt;
		...
		IF p_username IS NOT NULL THEN
		  	SELECT p_username as username, @fn as firstname, @on as email,@ph as photo, @lt as login_time FROM DUAL;
		END IF;
END $$
...
END $$
-- ----------------------------------------------------------
 -- modify just user photo's to use
-- ----------------------------------------------------------
...
CREATE PROCEDURE viewB_location(IN p_title VARCHAR(45))
BEGIN
	...
	SELECT title,edition,publisher, shelf_name
	FROM location L
...
-- ---------------------------------------------------------
-- update location of user when necessary
-- ---------------------------------------------------------
DROP PROCEDURE IF EXISTS modifybooklocat $$
CREATE PROCEDURE modifybooklocat(
	IN p_username VARCHAR(50),
	IN p_shelf_name VARCHAR(45))
BEGIN
	IF p_shelf_name != shelf_name THEN
		UPDATE location
...
-- ---------------------------------------------------------------------------
-- to modify book isbn and status mistake is made when putting
-- ---------------------------------------------------------------------------
...
BEGIN
	SELECT status, title, edition, publisher, category, photo
	FROM book B
	LEFT JOIN book_detail Bd
	ON Bd.book_isbn = B.isbn
	LEFT JOIN authors au
...
BEGIN
	INSERT INTO book_detail(`book_isbn`, `title`, `edition`, `publisher`, `category`,`photo`)
	VALUES(book_isbn, title, edition, publisher, category, photo);
END $$
...
BEGIN
	...
	VALUES(isbn, item_id, description);
END $$

-- --------------------------------------------------------------------------
-- modify cd inserted in the database
-- --------------------------------------------------------------------------
DROP PROCEDURE IF EXISTS modifyCd $$
CREATE PROCEDURE modifyCd (
	IN p_isbn INT,
	IN description VARCHAR(50))
BEGIN
	UPDATE cd
...
BEGIN
	SELECT status, cd_title, cd_edition, cd_publisher
	FROM book
	LEFT JOIN cd_detail C
	ON C.cd_detail_isbn
	LEFT JOIN cd D
	ON D.cd_isbn
	WHERE cd_category = p_category;
END $$
...
BEGIN
	SELECT status, cd_title, cd_edition, cd_publisher, cd_category
	FROM book B
	LEFT JOIN cd_detail C
	ON C.cd_detail_isbn;
END $$
...
BEGIN
	INSERT INTO cd_detail(`cd_detail_isbn`, `cd_title`, `cd_edition`, `cd_publisher`, `cd_category`)
	VALUES(cd_detail_isbn, cd_title, cd_edition, cd_publisher, cd_category);
END $$

-- ------------------------------------------------------------
-- modify cd info if put wrongly
-- ------------------------------------------------------------
